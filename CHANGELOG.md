...
- `app/companies/[slug]/page.tsx`, `app/compare/page.tsx`, `app/heatmap/page.tsx`, `app/timeline/page.tsx`, `components/compare-page-client.tsx`, `components/interactive-timeline.tsx`, `components/global-search.tsx`, `components/news-page-client.tsx`: Several routes depended on optional data without graceful empty handling. Hid empty sections and prevented unresolved loading or malformed content states from leaking into the UI.
...
- `lib/server-cache.ts`, `lib/db/queries.ts`, `lib/search/server.ts`, `app/api/health/route.ts`: The biggest CPU cost was repeated request-time rebuilding of the same news, leaderboard, company, digest, heatmap, timeline, health, and search datasets. Added shared cross-request caching with tagged invalidation, TTL-based reuse, and timing/cache-miss logs so read paths now serve stale-but-recent data instead of recomputing on every hit.
- `app/page.tsx`, `app/news/page.tsx`, `app/leaderboard/page.tsx`, `app/companies/page.tsx`, `app/companies/[slug]/page.tsx`, `app/compare/page.tsx`, `app/heatmap/page.tsx`, `app/timeline/page.tsx`: Major read surfaces were still forced dynamic. Switched them back to static/ISR rendering, and pre-generated tracked company profile routes so Vercel serves cached HTML instead of executing the server tree on every request.
- `components/news-page-client.tsx`, `components/compare-page-client.tsx`, `app/route-rendering.test.tsx`: The news and compare pages were only dynamic because the server read URL query params that the client could already handle. Moved those filters/selections to client-side URL parsing so the pages stay cacheable without losing existing deep-link behavior, and updated regression coverage to match the new static shell.
- `app/api/search/route.ts`, `lib/search/server.ts`: Search rebuilt company context and re-queried the DB on every identical request. Added short-lived response caching and cached search result reuse so repeated searches do less work.
- `components/data-freshness-indicator.tsx`, `components/news-page-client.tsx`: Open tabs were polling `/api/health` every minute with no cache benefit. Slowed those client refresh loops to every five minutes so background tabs stop churning serverless invocations.
- `app/api/cron/ingest/route.ts`, `app/api/cron/ingest-priority/route.ts`, `app/api/cron/recompute-leaderboard/route.ts`, `app/api/cron/backfill/route.ts`, `app/api/admin/ingest/route.ts`: Write paths updated content but left read caches waiting for TTL expiry. Added tag-based invalidation after successful ingestion/recompute runs so cached pages and APIs refresh immediately when data actually changes.
- `lib/ingestion/cron.ts`, `.github/workflows/frontier-pulse-scheduler.yml`, `app/page.tsx`, `app/about/page.tsx`: The scheduler was doing a full ingest every 30 minutes and a priority ingest every 30 minutes in between, which is wasteful on Hobby fluid compute. Reduced the cadence to hourly priority checks plus full ingestion every two hours, added cron duration logs, and updated the visible copy so the site’s stated refresh cadence matches reality.
...
- `components/scroll-reveal.tsx`: Homepage sections were relying entirely on `IntersectionObserver` to become visible, which could leave the body of the homepage looking blank until scroll or delayed hydration completed. Added an early near-viewport reveal check plus a short fallback timer so content is visible by default even when the observer is late.
